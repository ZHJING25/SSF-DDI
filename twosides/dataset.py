...
from test_representation import get_drug_embedding,get_drug_to_smiles
# %%
...
class DrugDataset(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        for row in batch:
            ...
            head_ids.append(get_drug_embedding(self.drug_to_smiles,Drug1_ID))
            tail_ids.append(get_drug_embedding(self.drug_to_smiles,Drug2_ID))
            head_ids.append(get_drug_embedding(self.drug_to_smiles,Neg_ID))
            tail_ids.append(get_drug_embedding(self.drug_to_smiles,Drug2_ID))
        ...
        return head_pairs, tail_pairs, rel, label,torch.stack(head_ids),torch.stack(tail_ids)
...
def load_ddi_dataset(root, batch_size, fold=0):
    ...
    train_loader = DrugDataLoader(train_set, batch_size=batch_size, shuffle=True, num_workers=4, drop_last=True)
    val_loader = DrugDataLoader(val_set, batch_size=batch_size, shuffle=False, num_workers=4)
    test_loader = DrugDataLoader(test_set, batch_size=batch_size, shuffle=False, num_workers=4)
